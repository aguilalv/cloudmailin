...
def test_campaign_classifier_handler_has_correct_steps():
    ...
    assert handler.steps == [
        assign_campaign_type
    ], "Handler should include only the assign_campaign_type step"
...
def test_campaign_classifier_handler_integration(
    ...
    assert (
        result.campaign_type == expected_campaign_type
    ), f"Expected campaign_type '{expected_campaign_type}' for {field}='{value}'"

    # Assert: Verify no other fields were changed
    for model_field in email.model_fields_set:
        if model_field != "campaign_type":
            assert getattr(result, model_field) == getattr(
                original_email, model_field
            ), f"Field '{model_field}' was unexpectedly modified"
