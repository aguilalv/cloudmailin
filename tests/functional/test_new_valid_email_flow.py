...
def test_email_processing_flow_with_handler(client, app, valid_real_payload, caplog, handler_class):
    """
    ...
    with app.app_context():
        ...
        # Assert: response
        assert response.status_code == 200, f"{handler_class.__name__} did not return 200 OK"

        # Assert: Log health check (check level and pattern)
        # Check dynamic log message
        assert any(
            record.levelname == "INFO" and f"[{handler_class.__name__}] Processing email" in record.message
